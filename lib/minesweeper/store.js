...
import {range, flatten, sample, compact} from "lodash";
...
const gameData = Rx.Observable.create((observer) => {
  const {xCount, yCount, mineCount} = options;
  const gameData = {
  ...
  function placeMines(toAvoidId) {
    sample(cells
      .filter((cell) => cell.id !== toAvoidId), mineCount)
      .forEach((cell) => {
        cell.mine = true;
      });
