...
export default class Minefield extends React.Component {
  render() {
    const {cellTable, cellWidth, cellHeight, xCount, yCount, revealCell, flagCell} = this.props;
    const style = {
      position: "relative",
    ...
    return (
      <div style={style}>
        {cellTable.map((row) => row.map((cell) =>
          <Minecell {...cell} key={cell.id} width={cellWidth} height={cellHeight} revealCell={revealCell} flagCell={flagCell}/>
        ))}
      </div>
    );
  }
